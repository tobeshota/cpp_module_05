...
// Intern::makeForm()の第一引数に"shrubbery creation"を指定するとShrubberyCreationFormクラスのオブジェクトが作られる
TEST(MakeFormTest, MakeShrubberyCreationFormTest) {
	...
	std::string expect = "_name: ShrubberyCreationForm\n_target: Bender\n_isSigned: 0\n_gradeToSign: 145\n_gradeToExec: 137\n";
...
TEST(MakeFormTest, MakeRobotomyRequestFormTest) {
	...
	std::string expect = "_name: RobotomyRequestForm\n_target: Bender\n_isSigned: 0\n_gradeToSign: 72\n_gradeToExec: 45\n";
	EXPECT_EQ(expect, actual);
}

// Intern::makeForm()の第一引数に"presidential pardon"を指定するとPresidentialPardonFormクラスのオブジェクトが作られる
TEST(MakeFormTest, MakePresidentialPardonFormTest) {
	...
	std::string expect = "_name: PresidentialPardonForm\n_target: Bender\n_isSigned: 0\n_gradeToSign: 25\n_gradeToExec: 5\n";
	EXPECT_EQ(expect, actual);
}
...
TEST(MakeFormTest, MakeFormThrowTest) {
	Intern someRandomIntern;
	EXPECT_THROW(someRandomIntern.makeForm("noForm", "Bender"), Intern::NoFormExpception);
}
